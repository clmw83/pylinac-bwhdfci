...
from pylinac.core.profile import CollapsedCircleProfile, SingleProfile
from pylinac.core.roi import DiskROI, RectangleROI, LowContrastDiskROI
...
class CatPhanBase:
    ...
    def results(self):
        ...
        #added for Excel output particular for BWH
        contrast = (1000+self.ctp404.hu_rois['Delrin'].pixel_value - (1000+self.ctp404.bg_hu_rois[3].pixel_value))/(1000+self.ctp404.hu_rois['Delrin'].pixel_value + 1000+self.ctp404.bg_hu_rois[3].pixel_value)
        cnr1 = np.sqrt(np.square(100*self.ctp404.hu_rois['Delrin'].std)+np.square(100*self.ctp404.bg_hu_rois[3].std))
        cnr2 = np.sqrt(np.square(1000+self.ctp404.hu_rois['Delrin'].pixel_value) + np.square(1000+self.ctp404.bg_hu_rois[3].pixel_value))
        cnr = cnr1/cnr2
        cnr = contrast/cnr
        uniformity = (1-self.ctp486.integral_non_uniformity)
        ...
        string2 = ('\n - Copy into Excel - \n'
                   ...
                   '{:2.4f}\n').format(self.ctp404.avg_line_length-50.0,
                                  ...
                                  contrast,                            
                                  cnr,
                                  self.ctp404.hu_rois['Air'].pixel_value,
                                  self.ctp404.hu_rois['LDPE'].pixel_value,
                                  self.ctp404.hu_rois['Acrylic'].pixel_value,
                                  self.ctp404.hu_rois['Teflon'].pixel_value,
                                  uniformity)  
        
        string2 += ('\n - Above values represent: \n'
                   'Geometric dis.\n'
                   'Spatial Res.\n'
                   'Contrast\n'
                   'CNR\n'
